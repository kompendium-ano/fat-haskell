...
import           FAT.RPC.Types.Balances
import           FAT.RPC.Types.Issuance
import           FAT.RPC.Types.Rates
import           FAT.RPC.Types.RichEntry
import           FAT.RPC.Types.SyncStatus
import           FAT.RPC.Types.Transaction
import           FAT.RPC.Types.TransactionStatus
...
-- NB: Factom and FAT API return results with `plain\text` headers, we
...
main = do
  ...
  (h, i) <- send s $ do
         h <- reqGetSyncStatus
         i <- reqFATIssuance
         --b <- reqFATBalances "FA38cwer93mmPw1HxjScLmK1yF9iJTu5P87T2vdkbuLovm2YXyss"
         --t <- reqGetTransaction "0-e4380e6334b0c42d4d6155fbd1378050b91c02a0df93d7fdfe6656f94c61e7eb"
         --r <- reqFATRates 213000
